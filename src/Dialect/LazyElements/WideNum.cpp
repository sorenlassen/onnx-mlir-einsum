...
APFloat WideNum::toAPFloat(BType tag) const {
  ...
  case BType::FLOAT16:
    return onnx_mlir::float_16(dbl).toAPFloat();
  case BType::BFLOAT16:
    return onnx_mlir::bfloat_16(dbl).toAPFloat();
  case BType::FLOAT8E4M3FN:
    return onnx_mlir::float_8e4m3fn(dbl).toAPFloat();
  case BType::FLOAT8E4M3FNUZ:
    return onnx_mlir::float_8e4m3fnuz(dbl).toAPFloat();
  case BType::FLOAT8E5M2:
    return onnx_mlir::float_8e5m2(dbl).toAPFloat();
  case BType::FLOAT8E5M2FNUZ:
    return onnx_mlir::float_8e5m2fnuz(dbl).toAPFloat();
  default:
